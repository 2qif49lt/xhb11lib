#ifndef XHBLIB_FUTURE_UTIL_H_
#define XHBLIB_FUTURE_UTIL_H_
...
using stop_iteration = safe_bool<stop_iteration_tag>;
...
future<> repeat(AsyncAction&& action) {
    ...
    try {
        do {
            ...
            if (f.get_first() == stop_iteration::yes) {
                return make_ready_future<>();
            }
        } while (!need_preempt());
...
template <typename T>
struct repeat_until_value_type_helper<future<optional<T>>> {
...
repeat_until_value(AsyncAction&& action) {
    ...
    do {
        ...
        optional_type&& optional = std::move(f).get_first();
        if (optional) {
            return make_ready_future<value_type>(std::move(optional.value()));
        }
    } while (!need_preempt());
...
when_all(Futs&&... futs) {
    using state = internal::when_all_state<internalsi::identity_futures_tuple<Futs...>, Futs...>;
...
when_all(FutureIterator begin, FutureIterator end) {
    using itraits = std::iterator_traits<FutureIterator>;
    using result_transform = internal::identity_futures_vector<typename itraits::value_type>;
...
struct reducer_with_get_traits<T, true> {
    ...
    static future_type maybe_call_get(future<> f, lstd::shared_ptr<T> r) {
        return f.then([r = std::move(r)] () mutable {
            return r->get();
...
map_reduce(Iterator begin, Iterator end, Mapper&& mapper, Initial initial, Reduce reduce) {
    ...
    while (begin != end) {
        ret = futurator::apply(mapper, *begin++).then_wrapped([s = s.get(), ret = std::move(ret)] (auto f) mutable {
            try {
                s->result = s->reduce(std::move(s->result), std::move(f.get_first()));
                return std::move(ret);
            } catch (...) {
